...
try {
  authKeys = JSON.parse(
    mustache.render(
      fs.readFileSync("./configurations/authKeys.json", "utf-8"),
      process.env,
      {},
      ["$_[", "]"]
    )
  ...
  governify
    ...
    .tag("startup")
    .info(
      "No configurations/scopeManager/authKeys.json found! Using default values."
    );
  // Minimal authKeys
  authKeys = {
    github: "",
      ...
      .then(() => {
        getPeriods(dsl)
          .then((periods) => {
            ...
            getScopeInfo(dsl.config.scopeManager, dsl.metric.scope)
              .then((response) => {
                const integrations = generateIntegrationsFromScopeInfo(
                  response.scope
                );
                ...
                calculateComputations(
                  ...
                  .catch((err) => {
                    logger.error(
                      "addComputation.calculateComputations:\n",
                      err
                    );
...
module.exports.getComputation = (computationID) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      if (
        results[computationID] !== undefined &&
        results[computationID] !== null
      ) {
...
const validateInput = (dsl) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      var iso8601Millis =
        /^(-?(?:[1-9][0-9]*)?[0-9]{4})-(1[0-2]|0[1-9])-(3[01]|0[1-9]|[12][0-9])T(2[0-3]|[01][0-9]):([0-5][0-9]):([0-5][0-9])(\\.[0-9]+)?\.([0-9][0-9][0-9])(Z)?$/;
    ...
    try {
      ...
      if (windowPeriod === "customRuleDaily") {
        ...
        while (keepGoing) {
          ...
          toDate = new Date(
            Date.parse(fromStr) + periodLength * 24 * 60 * 60 * 1000
          );
      ...
      if (
        dsl.metric.element.value !== undefined &&
        dsl.metric.element.value.traceback
      ) {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (scope) {
        reject(
          new Error(
            "Failed when obtaining project scope.\nProjectScopeId: " +
              scope.project +
              ", ClassScopeId: " +
              scope.class
          )
...
const generateIntegrationsFromScopeInfo = (scope) => {
  ...
  for (const identity of scope.identities) {
    ...
    for (const itemI of Object.keys(identity)) {
      if (itemI !== "source") {
        integrations[identity.source][itemI] = identity[itemI];
      }
    }
  }
  ...
  for (const credential of scope.credentials) {
    for (const itemC of Object.keys(credential)) {
      if (itemC !== "source") {
        integrations[credential.source][itemC] = credential[itemC];
      }
    }
  }
                ...
                .then((result) => {
                  if (!isNaN(result.metric)) {
                    ...
                    computations.push({
                      scope: resultScope,
                      period: {
                        from: period.originalFrom,
                        to: period.originalTo,
                      },
...
const offsetDate = (date, offset) => {
  return new Date(
    Date.parse(date) + offset * 24 * 60 * 60 * 1000
  ).toISOString();
};
