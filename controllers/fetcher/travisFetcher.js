...
const getInfo = (options) => {
  return new Promise((resolve, reject) => {
    ...
    getDataPaginated(options.public ? publicApiUrl + options.endpoint : privateApiUrl + options.endpoint, options.token, options.to).then((data) => {
...
const getDataPaginated = (url, token, to, offset = 0) => {
  return new Promise((resolve, reject) => {
    ...
    if (cached !== undefined && cacheDate !== undefined && Date.parse(to) < Date.parse(cacheDate)) {
      ...
      // Finds the dataArray
      let arrayName = '';
      Object.keys(cached).forEach((x) => { if (!x.includes('@')) arrayName = x; });
