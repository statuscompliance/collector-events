...
const ALL_REPOS_ENDPOINT_TYPES = [
  "newBranchesAllRepos",
...
const getInfo = (options) => {
  return new Promise((resolve, reject) => {
    if (ALL_REPOS_ENDPOINT_TYPES.includes(options.endpointType)) {
      ...
      fetcherUtils
        .requestWithHeaders(
          (options.gitlabApiBaseUrl || apiUrl) + "/projects",
          ...
          Promise.all(promises)
            .then((response) => {
              if (options.endpointType === "branchesUpdateRatioAllRepos") {
                ...
                resolve([
                  { updateRatio: numberUpdatedBranches / response.length },
                ]);
...
const getProjectInfo = (options) => {
  return new Promise((resolve, reject) => {
    getDataPaginated(
      (options.gitlabApiBaseUrl || apiUrl) + options.endpoint,
      options.token,
      options.to
    )
      .then((data) => {
        ...
        .then((data) => {
          if (data.length && data.length !== 0) {
            ...
            if (data.message === "Not Found") {
              reject(
                new Error(
                  "GitLab project not found or unauthorized. URL: " + requestUrl
                )
              );
            } else {
              reject(
                new Error(
                  "Problem when requesting to GitLab. URL: " + requestUrl
                )
              );
            }
          } else {
