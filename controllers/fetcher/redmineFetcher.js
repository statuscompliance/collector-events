        ...
        if (options.endpointType === 'inProgressIssuesByMember') {
          ...
          for (const issue of filteredData) {
            const promise = new Promise((resolve, reject) => {
              try {
                fetcherUtils.requestWithHeaders((options.redmineApiBaseUrl || apiUrl) + '/issues/' + issue.id + '.json?include=journals', { 'X-Redmine-API-Key': options.token }).then(data => {
            ...
            const promise = new Promise((resolve, reject) => {
              try {
                fetcherUtils.requestWithHeaders((options.redmineApiBaseUrl || apiUrl) + '/issues/' + issue.id + '.json?include=journals', { 'X-Redmine-API-Key': options.token }).then(data => {
                  ...
                  for (const journal of data.journals) {
                    for (const detail of journal.details) {
                      if (detail.name === 'status_id' && detail.new_value === '5') {
                  ...
                  if (inProgressJournal !== undefined) {
                    if (issue.status.id === 2) { // Current date - In progress date < 7 days
                      if ((new Date() - new Date(inProgressJournal.created_on)) / 86400000 < 7) {
                        result.push(issue);
                      }
                    } else if (issue.status.id === 5) { // Closing date - In progress date < 7 days
                      if ((new Date(closingJournal.created_on) - new Date(inProgressJournal.created_on)) / 86400000 < 7) {
                        result.push(issue);
                      }
                    }
                  }
...
const getDataPaginated = (url, token, offset = 0) => {
  return new Promise((resolve, reject) => {
    let requestUrl = url;
    requestUrl += requestUrl.split('/').pop().includes('?') ? '&limit=100&offset=' + offset : '?limit=100&offset=' + offset;
