...
const compute = (dsl, from, to, integrations, authKeys, member) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      const metricType =
        typeof element === typeof "" ? element : Object.keys(element)[0];
      ...
      if (metricType === "number") {
        ...
        if (percentageType === "related") {
          ...
          const window = relatedObject.window
            ? relatedObject.window
            : 86400 * 365 * 4; // 4 years window if not stated
          const secondaryEventFrom = new Date(
            Date.parse(from) - window * 1000
          ).toISOString();
          const secondaryEventTo = new Date(
            Date.parse(to) + window * 1000
          ).toISOString();

          getEventMatches(
            relatedObject,
            mainEvents,
            Object.keys(dsl.event[mainEventType])[0],
            secondaryEventFrom,
            secondaryEventTo,
            { ...integrations },
            authKeys,
            ...
            .then((eventMatches) => {
              ...
              if (metricType === "percentage") {
                metric =
                  (percentageNum / mainEvents[mainEventType].length) * 100;
        ...
        for (const item of mainEvents[mainEventType]) {
          try {
            ...
            if (value === undefined) {
              logger.error(
                "Fetcher.compute: Undefined value (",
                dsl.element.value.parameter,
                ") for item:\n",
                item
              );
            } else {
            ...
            logger.error(
              "Fetcher.compute: Could not extract value (",
              dsl.element.value.parameter,
              ") from item:\n",
              item
            );
        ...
        if (values.length === 0) {
          if (dsl.element.value.traceback) {
            // Should return always a value due to guarantee division
            metric = 0;
          } else {
            metric = NaN;
          }
        ...
        getPeriodBoundaries(from, to, dsl.element.stdev.period)
          .then((periodBoundaries) => {
            ...
            for (const event of mainEvents[mainEventType]) {
              const mainEventDate = Date.parse(
                sourcesManager.getEventDate(
                  mainEventType,
                  Object.keys(dsl.event[mainEventType])[0],
                  event
                )
            ...
            const mean =
              resultArray.reduce((a, b) => a + b, 0) / resultArray.length || 0;
            const variance =
              resultArray.reduce((a, b) => a + Math.pow(b - mean, 2), 0) /
...
const getEventMatches = (
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      const window = relatedObject.window;
      if (window !== undefined) {
        relatedKeys.splice(relatedKeys.indexOf("window"), 1);
      } // Remove window from the keys

      if (relatedKeys.length !== 1) {
        reject(
          new Error(
            "Too many arguments in related (expected 2: {api} and window)"
          )
        );
      } else {
        ...
        getEventsFromJson(
          ...
          .then((secondaryEvents) => {
            ...
            findMatches(
              ...
              secondaryEvents,
              secondaryEventType,
              Object.keys(relatedObject[secondaryEventType])[0],
              relatedObject,
              mainEventObject
            )
              .then((foundMatches) => {
      ...
      if (endpointType === "custom") {
        if (json[eventType].custom.type === "graphQL") {
          const steps = JSON.parse(
            JSON.stringify(json[eventType].custom.steps)
          );

          // Substitute query integrations
          Object.keys(steps)
            .filter((stepKeyElement) =>
              ["queryGetObject", "queryGetObjects"].includes(
                steps[stepKeyElement].type
              )
            )
            .forEach((queryKey) => {
              steps[queryKey].query = steps[queryKey].query
                .replace(
                ...
                .replace(
                  ...
                  integrations.github.repoOwner
                );
            });

          logger.debug(
            "Fetcher.getEventsFromJson: Performing GraphQL request to repository: ",
            integrations.github.repoOwner + "/" + integrations.github.repository
          );
          ...
          if (member) {
            ...
            for (const stepKey of Object.keys(steps).filter((stepKeyElement) =>
              [
                "objectsFilterObject",
                "objectsFilterObjects",
                "runScript",
              ].includes(steps[stepKeyElement].type)
            )) {
              if (steps[stepKey].type === "runScript") {
                // Substitute in the script code directly
                steps[stepKey].script = steps[stepKey].script.replace(
                  /%MEMBER\.github\.username%/g,
                  member.identities.filter((i) => i.source === "github")[0]
                    .username
                );
              } else {
                const newFilters = [];
                // For each filter with a regex match
                for (let filter of steps[stepKey].filters.filter(
                  (filterElement) => memberRegex.test(filterElement)
                )) {
                  // For each regex match in the filter
                  for (const regexMatch of filter.match(memberRegex)) {
                    const splitted = regexMatch
                      .replace(/%/g, "")
                      .replace("MEMBER.", "")
                      .split(".");
                    const identity = member.identities.filter(
                      (e) => e.source === splitted[0]
                    )[0];
                    if (identity) {
                ...
                steps[stepKey].filters = newFilters.concat(
                  steps[stepKey].filters.filter(
                    (filterElement) => !memberRegex.test(filterElement)
                  )
                );
              }
          ...
          logger.error(
            "Custom request type not valid: " + json[eventType].custom.type
          );
          resolve([]);
        }
      } else {
        // Endpoint generation from endpoints.json
        const endpoint = sourcesManager.getEndpoint(
          eventType,
          endpointType,
          integrations
        );
        if (endpoint === undefined) {
          ...
          const mustMatch = sourcesManager.getMustMatch(
            json[eventType][endpointType],
            integrations,
            member
          );
              ...
              case "travis":
                travisFetcher
                  .getInfo({
                    ...
                    to: to,
                    token: generateToken(
                      integrations.travis.apiKey,
                      endpointType.includes("private")
                        ? authKeys.travis_private
                        : authKeys.travis_public,
                      "token "
                    ),
                    endpoint: endpoint,
                    endpointType: endpointType,
              ...
              case "codeclimate":
                codeclimateFetcher
                  .getInfo({
                    ...
                    to: to,
                    token: generateToken(
                      integrations.codeclimate.apikey,
                      authKeys.codeclimate,
                      "Token token="
                    ),
                    endpoint: endpoint,
                    endpointType: endpointType,
    ...
    try {
      ...
      for (const mainEvent of mainEvents) {
        for (const secondaryEvent of secondaryEvents) {
          ...
          let secondaryEventDate = Date.parse(
            sourcesManager.getEventDate(
              secondaryEventType,
              secondaryEndpointType,
              secondaryEvent
          ...
          // Match filters
          const mainEventDSL =
            mainEventObject[mainEventType][
              Object.keys(mainEventObject[mainEventType])[0]
            ];
          const secondaryEventDSL =
            relatedObject[secondaryEventType][
              Object.keys(relatedObject[secondaryEventType])[0]
            ];

          const window = relatedObject.window
            ? relatedObject.window
            : 86400 * 365 * 10; // 10 years window if not stated (undefined window)
          if (
            Math.abs(mainEventDate - secondaryEventDate) / 1000 < window &&
            matchBinding(mainEvent, secondaryEvent, secondaryEventDSL) &&
            matchBinding(secondaryEvent, mainEvent, mainEventDSL)
          ) {
      ...
      if (matches.length > 0) {
        ...
        trimmedSecondaryEvents.splice(
          secondaryEvents.indexOf(matches[0][1]),
          1
        );
  ...
  try {
    ...
    for (const binding of bindings) {
      ...
      if (bindingType === "CONTAINS") {
        ...
        logger.error(
          "fetcher.matchBinding: Unknown binding type:",
          bindingType
        );
        res = false;
      }
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (periodLength === undefined) {
        reject(
          new Error(
            "metric.element.stdev.period must be within these: hourly, daily, weekly, biweekly, monthly, bimonthly."
          )
        );
      }
